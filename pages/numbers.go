...
func init() {
	...
	NumbersPage.Retarget("#container")
	NumbersPage.Wrap(wrapNumbers)
	NumbersPage.UseScripts("numbers.js").Trigger("numbers", "")
	NumbersPage.GetComponent("/one", gorouter.SimpleComponent(views.One)).UseScripts("one.js").Trigger("hi", "").Retarget("#number")
	NumbersPage.GetComponent("/two", gorouter.SimpleComponent(views.Two)).UseScripts("two.js").Trigger("bye", "").Retarget("#number")
